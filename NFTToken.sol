...
contract NFTToken is ERC721, Ownable {
    ...
    mapping(uint256 => uint256) private levelNum;
    ...
    constructor() public ERC721("TFI NFT", "TFI NFT") {
        _setBaseURI("https://nft.trustfi.org/");
    }
    ...
    function mint(address _to, uint256 _level) public auth {
        _tokenIds.increment();
        uint256 newItemId = _tokenIds.current();
        level[newItemId] = _level;
        _mint(_to, newItemId);
        levelNum[_level] = levelNum[_level] + 1;
    }
    ...
    function burn(uint256 _tokenId) public auth {
        ...
        levelNum[_level] = levelNum[_level] - 1;
        delete level[_tokenId];
    }

    function getNumOfLevel() public view returns (allNums memory nums) {
    ...
    function getNFTList(address _user, uint256 _page, uint256 _size) public view returns (uint256 total, NFTInfo[] memory list) {
        uint256 balance = balanceOf(_user);
        require(_page >= 1, "invalid param");
        require(_page.sub(1).mul(_size) <= balance, "invalid param");
        uint256 max;
        if (_page.mul(_size) >= balance) {
            max = balance;
        } else {
            max = _page.mul(_size);
        }
